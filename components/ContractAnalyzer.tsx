...
import { Upload, CheckCircle, ArrowRight, Loader2, Download, MessageSquare, GitCompare, Layers, Send, User, Bot, Shield, ChevronDown, Trash2, FileText, RefreshCw, X, AlertTriangle, Briefcase } from 'lucide-react';
...
const ContractAnalyzer: React.FC<ContractAnalyzerProps> = ({ context, preloadedAnalysis, onUpdateContext, onChangeView }) => {
  ...
  const addQuickRule = () => {
      const content = prompt("Eklenecek Geçici Madde / Kural:");
      if (!content || !onUpdateContext) return;
  ...
  const saveToHistory = (analysis: ContractAnalysisResult) => {
    const existing = localStorage.getItem('lexguard_history');
    const history = existing ? JSON.parse(existing) : [];
    const newHistory = [analysis, ...history.filter((h: any) => h.id !== analysis.id)].slice(0, 50);
  ...
  const handleAnalyze = async () => {
    ...
    const timeoutId = setTimeout(() => {
        if (isAnalyzingRef.current) {
            ...
            alert("İşlem çok uzun sürdü. Lütfen internet bağlantınızı kontrol edin veya daha kısa bir metin deneyin.");
        }
    }, 60000);
  ...
  const handleClearSession = () => {
    if(window.confirm('Mevcut analiz verileri temizlenecek ve başlangıç ekranına dönülecek. Onaylıyor musunuz?')) {
    ...
    return (
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm h-full flex flex-col overflow-hidden transition-colors">
        <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-slate-900 dark:bg-slate-950 text-white">
          ...
          <button onClick={downloadRevised} className="text-xs bg-white/10 hover:bg-white/20 px-3 py-1.5 rounded transition-colors flex items-center gap-1">
              <Download size={14} /> Temiz Metni İndir
          </button>
        </div>
                ...
                <div className="mt-4 flex gap-2">
                    ...
                    {contractText && (
                        <button 
                            ...
                            className="bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/40 px-4 rounded-lg font-bold flex items-center justify-center transition-colors border border-red-100 dark:border-red-800"
                            title="Metni Temizle"
                        >
                            <Trash2 size={18}/>
                        </button>
              ...
              <div className="flex justify-between items-center cursor-pointer" onClick={() => setShowRules(!showRules)}>
                  <div className="flex items-center gap-2">
                      ...
                      <div>
                          ...
                          <p className="text-[10px] text-amber-700/80 dark:text-amber-500/80">
                              Analize dahil edilecek ek maddeler veya geçici sözleşme metni.
                          </p>
                      </div>
                  </div>
                  ...
                  <div className="mt-4 space-y-3 animate-fade-in text-left border-t border-amber-200/50 dark:border-amber-800/50 pt-3">
                      ...
                      <div className="flex items-center justify-between bg-white dark:bg-slate-800 p-3 rounded-lg border border-amber-200 dark:border-amber-800/50 shadow-sm">
                          <span className="text-sm font-bold text-slate-700 dark:text-slate-300">Bu Geçici Sözleşmeyi Analize Dahil Et</span>
                      ...
                      <div className="bg-white dark:bg-slate-800 p-4 rounded-lg border border-amber-100 dark:border-amber-900/30 text-center">
                          <p className="text-sm text-slate-600 dark:text-slate-400 mb-3">
                              Geçici sözleşmeleri yüklemek, düzenlemek veya silmek için yönetim sayfasına gidin.
                          </p>
                          <button 
                          ...
                          >
                              <Briefcase size={16}/> Sözleşmeleri Yönet
                          </button>
                      </div>

                      {/* Summary of Active Contracts */}
                      <div className="text-xs text-slate-500 dark:text-slate-400 text-center mt-2">
                          Şu an {context.customRules?.filter(r => r.isActive && (r.isTemporaryContract || r.category === 'Geçici Sözleşme' || r.category === 'Geçici' || r.category === 'Ek Belge')).length || 0} adet geçici sözleşme maddesi analize dahil ediliyor.
                      </div>
                  </div>
             )}
         </div>
      ...
      {status === AnalysisStatus.ANALYZING && (
         <div className="flex-1 flex flex-col items-center justify-center space-y-4 animate-pulse">
            <div className="relative">
               <div className="w-20 h-20 border-4 border-slate-200 dark:border-slate-700 border-t-blue-600 dark:border-t-blue-500 rounded-full animate-spin"></div>
               <Shield className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-600" size={32}/>
            </div>
            <h3 className="text-xl font-bold text-slate-800 dark:text-white">AKINROBOTICS AI İnceliyor...</h3>
