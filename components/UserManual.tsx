...
const UserManual: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col p-6 md:p-12 overflow-y-auto bg-slate-50">
      <div className="max-w-6xl mx-auto w-full pb-20">
        <div className="mb-10 text-center animate-fade-in">
            <div className="inline-flex flex-col items-center justify-center p-6 bg-white rounded-3xl mb-6 shadow-xl shadow-slate-200 ring-1 ring-slate-100 min-w-[200px] min-h-[100px]">
                {imgError ? (
                   <div className="fallback-text text-3xl font-black text-slate-900 tracking-tighter">
                     AKINROBOTICS
                   </div>
                ) : (
                    <img 
            ...
            <h1 className="text-3xl md:text-5xl font-bold text-slate-900 mb-4 tracking-tight">Kullanım Kılavuzu & Özellikler</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
            {sections.map((section, idx) => (
                <div key={idx} className={`bg-white rounded-2xl border shadow-sm overflow-hidden hover:shadow-lg transition-all duration-300 group ${section.color || 'border-slate-200'}`}>
                    <div className="p-5 border-b border-slate-100 flex items-center gap-4 bg-slate-50/50">
                        <div className="p-3 bg-white rounded-xl border border-slate-100 shadow-sm text-slate-700 group-hover:scale-110 transition-transform duration-300">
                            {section.icon}
                        </div>
