...
const SmartFiller: React.FC<SmartFillerProps> = ({ context }) => {
  ...
  const handleApplyRiskDecisions = async () => {
      ...
      try {
          ...
          alert("Risk düzeltme sırasında hata oluştu. Orijinal metinle devam ediliyor.");
          setStep('QUESTIONS');
      }
  };
  ...
  const performFinalFill = async (customValues: Record<string, string>, opIdOverride?: number) => {
      ...
      try {
          ...
          setStep('DONE');
           ...
           alert("Belge doldurulurken hata oluştu.");
           setStep('UPLOAD');
      }
  };
  ...
  const handleCancel = () => {
      if(confirm("İşlemi iptal etmek istediğinize emin misiniz? Yüklenen belge ve veriler temizlenecektir.")) {
          startNewSession();
      }
  };
  ...
  const handleForceCancel = () => {
      operationIdRef.current += 1; 
      setStep('UPLOAD'); 
      alert("İşlem kullanıcı tarafından durduruldu.");
  };
  ...
  const renderUploadStep = () => (
    <div className="flex flex-col items-center justify-center h-full p-6 animate-fade-in">
        <div className="max-w-xl w-full bg-white p-8 rounded-2xl border-2 border-dashed border-slate-300 hover:border-indigo-400 transition-colors text-center shadow-sm">
            <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6 transition-all ${
                inputText ? 'bg-green-100 text-green-600' : 'bg-indigo-50 text-indigo-600'
            }`}>
            ...
            <p className="text-slate-500 mb-6">
                ...
                <span className="text-xs text-indigo-600 font-semibold">AI önce riskleri denetleyecek, sonra boşlukları dolduracaktır.</span>
            </p>
            ...
            <div className="flex flex-col items-center gap-3 w-full">
                <label className={`w-full max-w-xs cursor-pointer px-6 py-3 rounded-xl font-semibold transition-all shadow-sm items-center justify-center gap-2 flex ${
                    inputText ? 'bg-white border border-slate-300 text-slate-700 hover:bg-slate-50' : 'bg-slate-900 hover:bg-slate-800 text-white'
                }`}>
                ...
                {inputText && (
                     <button 
                        ...
                        className="w-full max-w-xs bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-xl font-bold shadow-lg shadow-indigo-200 transition-all flex items-center justify-center gap-2"
                     >
            ...
            {uploadedFileName && (
                <div className="mt-4 text-xs font-bold text-green-600 bg-green-50 px-3 py-1 rounded-full inline-flex items-center gap-1">
  ...
  const renderRiskReviewStep = () => (
      <div className="h-full flex flex-col max-w-4xl mx-auto w-full p-6 animate-fade-in">
          <div className="bg-white rounded-xl shadow-xl border border-red-100 overflow-hidden flex flex-col h-full relative">
               <div className="bg-red-50 p-6 border-b border-red-100 shrink-0 flex items-center gap-4">
                   <div className="w-12 h-12 bg-red-100 text-red-600 rounded-full flex items-center justify-center shrink-0">
                       <ShieldAlert size={28}/>
                   </div>
                   <div>
                        <h3 className="text-xl font-bold text-red-900">Risk Yönetim Paneli</h3>
                       ...
                       return (
                           <div 
                                ...
                                className={`border rounded-lg p-4 cursor-pointer transition-all relative ${
                                   isSelected 
                                   ? (risk.severity === 'High' ? 'border-red-400 bg-red-50 ring-1 ring-red-400' : 'border-indigo-400 bg-indigo-50 ring-1 ring-indigo-400')
                           ...
                           >
                               ...
                               <div className="flex justify-between items-center mb-2 pr-8">
                                   ...
                                   <span className={`text-xs font-bold px-2 py-1 rounded transition-colors ${
                                       isSelected ? 'text-green-700 bg-green-100' : 'text-slate-500 bg-slate-100'
                                   }`}>
                                       {isSelected ? 'DÜZELTİLECEK' : 'YOKSAYILACAK'}
                                   </span>
                               </div>
                               <p className="font-bold text-slate-800 text-sm mb-2 pr-8">{risk.description}</p>
                               <div className="flex gap-2 items-start bg-white/60 p-3 rounded border border-black/5 text-sm text-slate-600">
                                   <Sparkles size={16} className="text-indigo-500 shrink-0 mt-0.5"/>
                                   <div>
                                       <strong className="text-indigo-900">Öneri:</strong> {risk.suggestion}
                                   </div>
                               </div>
                           </div>
                       );
                   }) : (
                        <div className="text-center text-slate-500">Risk bulunamadı veya veri yüklenemedi.</div>
                   )}
               </div>

               <div className="p-6 bg-slate-50 border-t border-slate-200 shrink-0 flex flex-col md:flex-row gap-4 justify-between items-center">
      ...
      <div className="h-full flex flex-col max-w-3xl mx-auto w-full p-6 animate-fade-in">
          <div className="bg-white rounded-xl shadow-lg border border-indigo-100 overflow-hidden flex flex-col h-full">
                      ...
                      missingFields.map((field, idx) => (
                        <div key={idx} className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
                            ...
                            <input 
                                ...
                                className="w-full p-3 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none font-medium text-slate-900 placeholder-slate-400 shadow-sm transition-all hover:border-indigo-300"
                            />
                        </div>
                      ))
                  )}
              ...
              <div className="p-6 bg-white border-t border-slate-200 shrink-0 flex justify-between items-center">
                   <button onClick={handleCancel} className="text-slate-500 hover:text-slate-800 text-sm font-bold">İptal</button>
                   <button 
      ...
      <div className="h-full flex flex-col p-6 animate-fade-in gap-6">
          <div className="flex justify-between items-center shrink-0">
              <div>
                  ...
                  <p className="text-slate-500 text-sm">Güvenlik kontrolünden geçti ve dolduruldu.</p>
              </div>
              <div className="flex gap-2">
                  <button onClick={startNewSession} className="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-lg font-bold border border-slate-200 transition-colors">
                      Yeni Belge
                  </button>
                  <button 
                  ...
                  <button 
                    ...
                    className="px-6 py-2 bg-blue-600 text-white hover:bg-blue-700 rounded-lg font-bold shadow-md transition-all flex items-center gap-2 min-w-[160px] justify-center"
          ...
          <div className="flex-1 overflow-y-auto space-y-6">
              ...
              {/* FINAL SÖZLEŞME */}
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-0 overflow-hidden flex flex-col min-h-[500px]">
                  <div className="bg-slate-50 px-4 py-2 border-b border-slate-200 text-xs font-bold text-slate-500 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                      <div className="flex items-center gap-4">
                          ...
                          <div className="flex bg-slate-200 rounded-lg p-0.5">
                              <button 
                                  ...
                                  className={`flex items-center gap-1 px-3 py-1 rounded-md text-xs font-bold transition-all ${!isEditMode ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                              >
                      ...
                      {isEditMode && (
                          <span className="flex items-center gap-1 text-blue-600"><PenTool size={12}/> Düzenlenebilir</span>
                      )}
                  </div>
                  ...
                  {isEditMode ? (
                      <textarea 
                          ...
                          className="flex-1 w-full p-8 resize-none outline-none font-serif text-sm text-slate-900 bg-white leading-relaxed focus:bg-slate-50/50 transition-colors"
                          placeholder="Belge oluşturuluyor..."
                      />
                  ) : (
                      <div className="flex-1 w-full p-8 font-serif text-sm text-slate-900 bg-white leading-relaxed overflow-y-auto whitespace-pre-wrap">
              ...
              {(changeLog || (fillLog && fillLog.length > 0)) && (
                  <div className="bg-red-50 border-2 border-red-200 rounded-xl p-0 shadow-sm overflow-hidden">
                      <div className="bg-red-100/50 p-4 border-b border-red-200 flex items-center justify-between">
                          <h4 className="text-red-900 font-bold text-lg flex items-center gap-2">
                             <ShieldCheck size={24}/> AI Hukuk Denetim Paneli
                          </h4>
                          <span className="text-red-700 text-xs font-bold uppercase tracking-wide">Değişiklik Günlüğü</span>
                      </div>
                      ...
                      <div className="p-6 space-y-6">
                          {changeLog && (
                             <div>
                                 ...
                                 <div className="bg-white border border-red-100 rounded-lg p-4 prose prose-sm prose-red max-w-none text-slate-800">
  ...
  return (
    <div className="h-full w-full bg-slate-50 relative">
        ...
        {(step === 'ANALYZING' || step === 'FILLING' || step === 'FIXING_RISKS') && (
            <div className="absolute inset-0 z-50 bg-white/95 backdrop-blur-md flex flex-col items-center justify-center animate-fade-in px-4 text-center">
                
                {/* --- TIME INDICATOR --- */}
                <div className="mb-8 w-full max-w-md">
                     <div className="flex justify-between text-xs font-bold text-slate-500 mb-2">
                         <span className="flex items-center gap-1"><Clock size={12}/> İşlem Süresi</span>
                         <span className={remainingSeconds < 20 ? "text-red-600 animate-pulse" : "text-slate-700"}>
                ...
                {step === 'FIXING_RISKS' ? (
                     <Sparkles size={64} className="text-indigo-600 animate-pulse mb-6"/>
                ) : (
                ...
                {/* --- LIVE LOG TERMINAL --- */}
                <div className="w-full max-w-lg bg-slate-900 rounded-lg p-4 mb-8 text-left font-mono text-xs shadow-xl border border-slate-700 max-h-48 overflow-y-auto custom-scrollbar">
                ...
                <button 
                    ...
                    className="flex items-center gap-2 px-6 py-3 bg-red-50 text-red-600 border border-red-200 rounded-xl hover:bg-red-100 transition-colors font-bold shadow-sm"
                >
                    <XCircle size={20}/>
                    Durdur ve İptal Et
                </button>
        ...
        {isContextMissing && step === 'UPLOAD' && (
           <div className="absolute top-4 right-4 z-40 max-w-sm bg-amber-50 border border-amber-200 p-4 rounded-xl flex items-start gap-3 text-amber-900 shadow-lg">
