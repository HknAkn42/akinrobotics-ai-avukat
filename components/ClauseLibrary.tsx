    ...
    <div className="h-full flex flex-col p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-blue-50 border border-blue-100 p-3 rounded-lg flex items-center gap-3">
              <div className="bg-blue-100 p-2 rounded text-blue-600"><Zap size={18}/></div>
          ...
          <div className="bg-green-50 border border-green-100 p-3 rounded-lg flex items-center gap-3">
              <div className="bg-green-100 p-2 rounded text-green-600"><ShieldCheck size={18}/></div>
          ...
          <div className="bg-purple-50 border border-purple-100 p-3 rounded-lg flex items-center gap-3">
              <div className="relative flex-1">
                ...
                <input 
                    ...
                    className="w-full pl-9 p-2 bg-white border border-purple-200 rounded-lg outline-none focus:ring-2 focus:ring-purple-500 text-sm"
                />
            </div>
          </div>
      </div>
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 overflow-y-auto">
        {filteredClauses.map(clause => (
          <div key={clause.id} className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all flex flex-col group relative">
            <div className="flex justify-between items-start mb-3">
              <span className="bg-teal-50 text-teal-700 text-[10px] px-2 py-1 rounded font-bold uppercase tracking-wider border border-teal-100">{clause.category}</span>
              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity absolute top-3 right-3 bg-white pl-2 shadow-sm rounded-lg border border-slate-100 p-1">
                <button 
            ...
            <h3 className="font-bold text-slate-800 mb-2 pr-16 text-sm">{clause.title}</h3>
            <div className="bg-slate-50 p-3 rounded-lg border border-slate-100 flex-1 relative group-hover:bg-slate-100 transition-colors">
                <p className="text-xs text-slate-600 leading-relaxed font-mono line-clamp-6">{clause.content}</p>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
          <div className="bg-white rounded-xl max-w-lg w-full p-6 space-y-4 shadow-2xl">
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold text-slate-700">Madde Başlığı</label>
              <input 
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold text-slate-700">İçerik (Tam Metin)</label>
              <textarea 
                rows={8}
                className="w-full border border-slate-300 p-2.5 rounded-lg font-mono text-sm focus:ring-2 focus:ring-teal-500 outline-none" 
            ...
            <div className="flex justify-end gap-2 pt-4 border-t">
              <button onClick={() => setEditingClause(null)} className="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-lg font-medium transition-colors">İptal</button>
              <button onClick={handleSave} className="px-6 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 flex items-center gap-2 font-bold shadow-lg shadow-teal-200 transition-all">
                <Save size={18} /> Kaydet
              </button>
            </div>
          </div>
