...
export const validateApiKey = async (apiKey: string, provider: AIProvider): Promise<{ isValid: boolean; message: string }> => {
  ...
  if ((!apiKey || apiKey.trim().length === 0) && provider === 'GOOGLE' && process.env.API_KEY) {
...
const generateUnifiedContent = async (req: UnifiedRequest): Promise<string> => {
    ...
    try {
        ...
        if (provider === 'GOOGLE') {
            const ai = new GoogleGenAI({ apiKey });
            const modelName = context.preferredModel || 'gemini-2.5-flash-latest'; // Default to fast model
        ...
        // 2. OPENAI
        if (provider === 'OPENAI') {
            const modelName = context.preferredModel || 'gpt-4-turbo';
            const messages = [
        ...
        if (provider === 'ANTHROPIC') {
            const modelName = context.preferredModel || 'claude-3-opus-20240229';
            const finalSystem = systemInstruction + (jsonMode ? " RETURN ONLY JSON. Do not include any explanation." : "");
...
const formatCustomRules = (context: CompanyContext): string => {
    if (!context.customRules || context.customRules.length === 0) return "";
    
    const activeRules = context.customRules.filter(r => r.isActive);
    if (activeRules.length === 0) return "";

    return `\n--- GEÇİCİ SÖZLEŞME VE EK PROTOKOL MADDELERİ (BU KURALLAR ANA SÖZLEŞMEYE EKLENMİŞTİR VE ÖNCELİKLİDİR) ---\n` + 
...
export const analyzeContract = async (
  ...
  try {
    ...
    const optimizedContext = {
        ...context,
        preferredModel: context.preferredModel?.includes('flash') ? context.preferredModel : 'gemini-2.5-flash-latest'
    };
